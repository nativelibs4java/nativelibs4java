...
public class ImageTest extends AbstractCommon {
	...
	public void testRGBAImageSource() {
			...
			//CLIntBuffer cloutput = context.createIntBuffer(CLMem.Usage.Output, width * height * 4);
            CLFloatBuffer cloutput = context.createFloatBuffer(CLMem.Usage.Output, width * height * 4);
            ...
            for (ChannelOrder channelOrder : Arrays.asList(ChannelOrder.BGRA, ChannelOrder.RGBA)) {
                ...
                FloatBuffer output = cloutput.read(queue);
                for (int x = 0; x < width; x++) {
                    for (int y = 0; y < height; y++) {
